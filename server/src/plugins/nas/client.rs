                ...
                "SYNC_DEVICE" => {
                        ...
                        .find(|s| s.device_name == *device_name)
                    {
                        if device_remote_modify_time < last_sync_info.last_sync_time {
                            info!(
                                &msg_tx_clone,
                                format!("[{NAME}] [Skip] {device_name} is already synced.")
                            );
                            continue;
                        } else {
                    ...
                    // send files_data
                    {
                        ...
                        info!(
                            &msg_tx_clone,
                            format!(
                                "[{NAME}] [Go] Send files_data to: {device_name}:{device_tailscale_ip}"
                            )
                        );
                        ...
                        info!(
                            &msg_tx_clone,
                            format!(
                                "[{NAME}] [Ok] Send files_data to: {device_name}:{device_tailscale_ip}"
                            )
                        );
                    }
